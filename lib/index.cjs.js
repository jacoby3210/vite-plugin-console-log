...
function composeConsoleLog(components) {
  ...
  return `${prefix + generateLogTitle() + generateFileLocation(fileRelativePath) + generateLine(lineCount) + (jump ? generateAddress(port, encodeURIComponent(`${fileAbsolutePath}:${lineCount}:${endCloumn}`)) : "") + `${generateNewLine()},${generateLogTitleStyle()},${generateFileLocationStyle()},${generateLineStyle()},` + (jump ? `${generateAddressStyle()},` : "") + suffix}`;
}
